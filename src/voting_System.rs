pub mod voting_system{
    ...
    impl VotingSystem{
        ...
        fn read_votes(&mut self)->Result<(), Error> {
            ...
            //error variable that will be passed if a error is found. Will be overwritten, just has
            //a default value for defining sake.
    ...
    impl VotingSystem{
        ...
        fn tally_votes(&mut self) {
            ...
            if pass==true {
                println!("----TOTAL VOTES PER COUNTY (FIRST ROUND)-----");
                println!("\tCOUNTY CODE\t A  B  C  D  E");
                let mut i = 1;
                while i<124 {
                    println!("\t\t{i}\t");
                    for u in 0..5 {
                        print!("  {}",self.county_votes[i][u]);
                    }
                    i+=2;
                }
